...
-- # Repeated Counted (Distinct)
Select Distinct( YEAR(paymentDate)) from payments;

-- # Grouping ke Liya (Group by) 
Select YEAR(paymentDate)  from payments group by YEAR(paymentDate);

-- # Exist Karta hai (Having)
select max(amount) as max_amount, Year(paymentDate) from payments group by YEAR(paymentDate) HAVING max_amount > 116209;
